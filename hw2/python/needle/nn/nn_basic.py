...
class SoftmaxLoss(Module):
    def forward(self, logits: Tensor, y: Tensor):
        ...
        # Each side of the loss is averaged by the batch size, and then add up to a scalar.
        lhs = ops.summation(ops.logsumexp(logits, (1,)) / logits.shape[0])
        rhs = ops.summation(y_one_hot * logits / logits.shape[0])
        return lhs - rhs 
        ### END YOUR SOLUTION
...
class LayerNorm1d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ### BEGIN YOUR SOLUTION
        # The raw mean is a plain 1D array, thus reshape is needed before broadcasting.
        ...
        # eps attached in case of div_by_zero error.
