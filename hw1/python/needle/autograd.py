...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        # Sum up partial adjoints of current node to be the node gradient
        node.grad = sum_node_list(node_to_output_grads_list[node])
        if node.op is None: # leaf node, no computation before
            continue
        partial_adjoints = node.op.gradient_as_tuple(node.grad, node)
        # Each node correponds to the partial adjoint in position.
        for in_node, partial_adjoint in zip(node.inputs, partial_adjoints):
            if in_node not in node_to_output_grads_list:
                node_to_output_grads_list[in_node] = list()
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    # dfs: visit first, then deal with the node itself 
    # Applying dfs will lead to the node relied by others appearing later in the list.
